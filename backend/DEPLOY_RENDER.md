...
| `CORS_ORIGIN` | آدرس فرانت‌اند روی Vercel — بعد از دیپلوی فرانت پر می‌شه (پایین رو ببینید) |

`TELEGRAM_BOT_TOKEN` رو فقط اگه از `botListener.js` (جایگزین ساده‌ی بات) به‌جای اتصال با اکانت شخصی استفاده می‌کنید لازم دارید.

> نقشه‌ی کلی ترتیب هر سه سرویس (Supabase → Render → Vercel) و سیم‌کشی بینشون در [`../DEPLOY.md`](../DEPLOY.md).

## بعد از دیپلوی فرانت‌اند: ست کردن `CORS_ORIGIN`
وقتی فرانت‌اند روی Vercel بالا اومد و آدرسش رو گرفتید (مثلاً `https://…​.vercel.app`)، برگردید اینجا و `CORS_ORIGIN` رو برابر با همون آدرس بذارید. بدون این، مرورگر همه‌ی درخواست‌های فرانت به بک‌اند رو به‌خاطر CORS بلاک می‌کنه. (جزئیات در [`../frontend/DEPLOY_VERCEL.md`](../frontend/DEPLOY_VERCEL.md).)
